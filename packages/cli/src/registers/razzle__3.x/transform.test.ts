...
const goodCodes: Record<string, string> = {
  ['3.1.0']: shelljs.cat(path.join(__dirname, 'fixtures/razzle-bin-razzle@3.1.0.txt')).stdout,
  ['3.4.5']: shelljs.cat(path.join(__dirname, 'fixtures/razzle-bin-razzle@3.4.5.txt')).stdout,
};
...
describe('transform', () => {
  describe.each(Object.keys(goodCodes))('%s', (codeFlag: string) => {
    describe.each(subcommands)('razzle %s', (subcommand: string) => {
